...
class VideoSplitterDialog(BaseDialog):
    def __init__(self, parent=None):
        ...
        self.file_progress_bar = QtWidgets.QProgressBar(); self.file_progress_label = QtWidgets.QLabel("Status: Idle")
        ...
        form_layout.addWidget(QtWidgets.QLabel("Video Files to Split:"), 0, 0); form_layout.addWidget(self.video_list_widget, 1, 0, 1, 2); form_layout.addLayout(file_buttons_layout, 1, 2)
        ...
        file_progress_layout = QtWidgets.QHBoxLayout(); file_progress_layout.addWidget(self.file_progress_label, 1)
        ...
        self.browse_output_btn.clicked.connect(self.browse_output); self.start_btn.clicked.connect(self.start_splitting); self.cancel_btn.clicked.connect(self.cancel_splitting)
    ...
    def start_splitting(self):
        ...
        self.splitter_worker = VideoSplitter(self.video_files, self.output_dir_line_edit.text(), self.chunk_duration_spinbox.value(), self.subfolder_checkbox.isChecked())
        self.splitter_thread = QThread(); self.splitter_worker.moveToThread(self.splitter_thread)
